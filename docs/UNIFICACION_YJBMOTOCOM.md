...
**Sincronizado con YBMOTOCOM el mismo día** (`96f2c93` → `5ba4000`, ver `docs/EXPANSION_SEGUNDA_TIENDA.md` sección 10) — con un conflicto (el único de los 4 casos de sincronización hasta ahora): `docs/UNIFICACION_YJBMOTOCOM.md` no existe en el repo de YBMOTOCOM, así que el cherry-pick lo marcó como conflicto "modificado / borrado"; se resolvió manteniéndolo borrado allá (`git rm`) sin afectar el resto del commit. **Pendiente crítico**: la migración `00055` todavía no se ha corrido en el Supabase de YBMOTOCOM (son bases de datos independientes) — hasta que eso pase, cualquier venta por Datáfono en esa tienda fallará porque `create_pos_sale` ya referencia una tabla que ahí no existe todavía.
