...
### 81.15 Meta mensual de ventas: de individual a meta compartida del equipo (2026-08-22)

El usuario notó que la tarjeta "Tu meta de ventas este mes" del Dashboard (sección 81.12) mostraba $310.000, un valor muy distinto a los $16.526.000 de Historial Mensual del mismo mes, y preguntó de dónde salía. No era un bug: la tarjeta sumaba solo las ventas de mostrador del vendedor que tenía la sesión abierta (`seller_id` propio), mientras que Historial Mensual suma el total de la tienda (todos los canales, todos los vendedores) — dos métricas distintas por diseño, tal como se documentó en 81.12.

Al explicarle la diferencia, el usuario pidió cambiar el enfoque: no quiere una meta individual por vendedor (que puede generar competencia), sino una meta **compartida por todo el equipo**, con el desglose de las ventas de cada vendedor visible para todos (no solo para el admin, que ya tenía esa vista en "Rendimiento Vendedores").

**Implementación**:
- `GET /api/reports/my-sales` → renombrado a `GET /api/reports/team-sales`: ya no filtra por `seller_id = auth.user.id`, trae **todas** las órdenes de mostrador pagadas del mes (mismo patrón de agrupación por vendedor que `seller-performance`, incluyendo vendedores con $0 en ventas) y devuelve tanto el desglose por persona como el total combinado del equipo. La meta/bono se sigue comparando contra ese total combinado, no contra el de cada quien. Sigue abierto a `admin` y `seller` por igual — ninguno de los dos roles es admin-only aquí.
- `MySalesGoalCard` → renombrado a `TeamSalesGoalCard`: ya no se oculta para el rol admin (antes solo la veía `seller`); ahora la ven todos, con la misma barra de progreso del total del equipo más una lista con la barra individual de cada vendedor.
- No se tocó el esquema: `seller_monthly_goal_cents`/`seller_goal_bonus_cents` en `store_settings` (migración 00045) ya representaban "la meta" en genérico, sin asumir alcance individual — solo cambió cómo se calcula el acumulado contra el que se compara.
- Texto de ayuda en `/admin/configuracion-pos` actualizado para reflejar que la meta es del equipo, no de cada vendedor.

`tsc`/`eslint` limpios. 127/129 tests en verde — los 2 que fallan (`loans.test.ts`) son preexistentes y no relacionados (confirmado corriéndolos también sobre el árbol sin este cambio).

### 81.16 Los 82 cascos viejos seguían apareciendo (Agotado) en Registrar Venta (2026-08-24)
...
### 81.18 "Stock Bajo" del Dashboard mostraba 5 cuando en realidad eran 105 (2026-08-26)

El usuario pidió eliminar "Casco Integral Pro Racing" (creado de pruebas, nunca vendido) y notó que la tarjeta "Stock Bajo" del Dashboard decía 5 mientras que Inventario mostraba más productos por debajo del mínimo — específicamente IMPERMEABLE SILICONADO NEGRO AZUL (1 unidad consolidada, mínimo 5) no aparecía en la alerta.

**Casco Integral Pro Racing**: verificado sin ninguna referencia real (0 `product_variants`, 0 `order_items`, 0 `inventory_movements`, 0 `restock_subscriptions`, 0 `product_reviews`, 0 `wishlists`) — ya estaba `deleted_at` desde el 2026-08-24 (uno de los 5 cascos reales de la limpieza de la sección 81.13/81.16), pero seguía "vivo" para el Dashboard por el bug de abajo. Con confirmación del usuario, se borró la fila por completo de `products` (no solo soft-delete) al no tener ninguna referencia que perder.

**Tres bugs reales en `getLowStockProducts()` (`admin/page.tsx`), todos en la misma función**:
1. **No filtraba `deleted_at`** — un producto ya eliminado desde Productos seguía contando en la alerta para siempre (por eso el Casco, aunque borrado, seguía apareciendo).
2. **Comparaba cada talla contra su propio umbral** en vez de sumar el stock de todas las variantes y compararlo contra el umbral del producto — inconsistente con `/admin/inventario`, que sí consolida. Un producto con una sola talla con existencia mínima podía no sonar la alerta si esa talla nunca cruzaba su propio umbral, aunque el total del producto estuviera crítico (el caso de IMPERMEABLE SILICONADO NEGRO AZUL).
3. **El número de la tarjeta era el tamaño de la lista de vista previa, no el conteo real** — la función recortaba a los primeros 5 resultados con `.slice(0, 5)` antes de que `DashboardTabs` usara `.length` para el número grande de la tarjeta. El "5" nunca fue un conteo, era el límite de la vista previa.

**Se corrigieron los tres** en `getLowStockProducts()`: ahora excluye `deleted_at`, consolida el stock por producto sumando `product_variants` (igual que Inventario) y devuelve la lista completa (el recorte a 5 se mueve a `DashboardTabs`, que además ahora muestra un link "Ver todos (N)"). Se encontró y corrigió el mismo bug de raíz en `sendLowStockAlert()` (`lib/email.ts`) — el correo automático que se dispara tras cada pago usaba `products.stock_qty` directo (siempre 0 en productos con tallas), así que en teoría cualquier producto con variantes activaba esa alerta por correo después de cada venta, sin relación con su stock real.

**Además**, ambas tarjetas "Stock Bajo" del Dashboard (pestaña Ventas y pestañas de canal) ahora son clickeables — enlazan a `/admin/inventario?stockBajo=1`, que activa el filtro "Stock bajo" y cambia a la pestaña Detalle automáticamente (leído de `window.location.search` en un `useEffect`, sin forzar un Suspense boundary en todo el componente).

**Verificado**: `tsc`/`eslint` limpios, 129/129 tests en verde. Consulta directa a producción confirmó que la lógica nueva del Dashboard y la de `/admin/inventario` devuelven exactamente el mismo conjunto de 105 productos (antes de la corrección, la función solo exponía 5). Verificado además con navegador real (Playwright, sesión admin real): el Dashboard mostró "Stock Bajo: 105", el click llevó a `/admin/inventario?stockBajo=1` con el filtro ya activado y la tabla mostrando esos mismos 105 productos, IMPERMEABLE SILICONADO NEGRO AZUL incluido. El umbral mínimo de casi todos estos productos está en el valor por defecto (5) sin ajustar por producto — vale la pena revisarlo, pero es una decisión de negocio, no un bug.

### 81.19 Alta de 4 impermeables "Puizzance" + confirmación y aplicación de la regla de redondeo del precio de venta a todo el catálogo (2026-08-27)

El usuario pidió dar de alta 4 impermeables nuevos (mismo modelo "Puizzance", 4 colores) con sus tallas y cantidades reales, con costo $115.000 c/u: NEGRO CON MORADO (XS=3,S=3,M=3), NEGRO CON ROSADO (XS=3,S=3,M=3), NEGRO CON ROJO (M=4,L=4,XL=4,2XL=4) y NEGRO (M=4,L=4,XL=4,2XL=4) — 50 unidades en total. Preguntó, de paso, cómo se redondea el precio de venta calculado con la fórmula ya confirmada (costo÷0,70, margen real del 30%, ver sección 81.13): $115.000÷0,70 = $164.285,71, ¿pero se muestra como $165.000?

**Hallazgo antes de responder**: se verificó contra el precedente real (los 24 cascos de la 81.13, creados con la misma fórmula) que esos productos quedaron con el valor EXACTO en pesos, sin redondear a miles (ej. un casco dio $578.285,71 y quedó en $578.286) — nunca existió una regla de "redondear a miles" en el catálogo. Se le presentó esto al usuario, quien decidió una regla **nueva**: redondear siempre **hacia arriba al millar más cercano** ($164.285,71 → $165.000), y aplicarla no solo a los 4 productos nuevos sino a **todo el catálogo existente con costo registrado**.

**Segundo hallazgo, antes de tocar precios en producción**: un *dry run* completo (sin escribir nada) sobre los 219 productos mostró que 132 tienen costo registrado, y aplicar la fórmula a ciegas habría **bajado el precio de 53 de ellos** (productos que hoy ya tienen más del 30% de margen real, ej. varios cascos con ratio ~1,50). Se le mostró esto al usuario explícitamente antes de escribir nada — no era visible cuando pidió "aplicar a todo ya" la primera vez. Decisión final: aplicar la fórmula **solo cuando sube el precio actual, nunca cuando lo baja** (deja intacto el margen de los 53, y corrige de paso casos claramente mal cargados, como IMPERMEABLE SILICONADO ROSADO que estaba en $7.500 — un error de digitación evidente frente a sus hermanos de color a $49.400-$60.000 — y quedó en $55.000).

**Ejecución** (script de una sola vez, con dry-run corrido y revisado antes de escribir — mismo patrón que la migración de Cascos): se subió el precio de **76 productos** (de los 132 con costo, los otros 56 ya estaban en o por encima de la fórmula) — backup de los 76 valores anteriores guardado fuera del repo para poder revertir si hiciera falta. Se crearon los 4 productos nuevos con `active=false` (pendientes de foto/descripción, igual que todo alta nueva desde "Ingresar"), `category_id` de Impermeables, costo $115.000 y precio $165.000 en los cuatro, con sus tallas como `product_variants` y códigos de barra autogenerados con el mismo algoritmo de "Ingresar" (`lib/inventario-barcode.ts`, portado al script) — 4 modelos nuevos consecutivos dentro de la categoría 16 (Impermeable), SKU 27-30 (siguientes al máximo real de 26).

**Verificación**: 50 unidades exactas repartidas en las 14 variantes de los 4 productos nuevos (coincide con lo pedido); `stock_qty` de cada producto sincronizado correctamente por el trigger de la Fase 1 (suma de sus variantes); 0 códigos de barra y 0 SKU duplicados en todo el catálogo tras la operación. Pendiente para el usuario: los 4 productos nuevos están sin foto/descripción y quedaron `active=false` a propósito — hay que completarlos y activarlos desde Inventario antes de que aparezcan en la tienda pública. Regla nueva para el futuro: el precio de venta de cualquier producto nuevo con margen real del 30% se calcula como costo÷0,70 **redondeado hacia arriba al millar de pesos más cercano** (no al valor exacto, como se hizo antes con los cascos).

### 81.20 Margen real de SisteCrédito en Cuentas + deuda faltante del 15 de agosto (2026-08-27)

El usuario notó que la lista de "Por Cobrar" de SisteCrédito en Cuentas le faltaba la deuda del 15/08/2026 ($250.000) y aprovechó para explicar una regla de negocio que nunca se había modelado: SisteCrédito retiene el pago casi 2 meses y cobra a la tienda una comisión real del 4%, pero la tienda le traslada al cliente un recargo mayor (6%, sin que el cliente lo sepa) — la diferencia (2%) es ganancia real que hoy no aparece en ningún lado, porque la venta se registra por el valor base (ej. $250.000) y lo que en verdad va a depositar SisteCrédito, una vez descuenta su 4%, es base+2%. Pidió mostrar esto en Cuentas: el total con el 2% en Resumen, y el desglose (base, +2%, total) en el detalle de Por Cobrar.

**Contexto verificado antes de tocar nada** (el usuario advirtió que "hace poco se hizo una modificación de eso"): la pestaña "Por Cobrar" es reciente (commit `2725871`, 2026-08-26, migración `00048`) — son filas manuales por cuenta (`account_receivables`: deudor, monto, fecha, notas), puramente informativas, sin ningún cálculo de porcentaje todavía. Se confirmó además que `pos_commission_rates.sistecredito` (Configuración POS → "Comisión por método de pago", usado en Historial Mensual/Reportes para la Utilidad Real) está en 0% — un concepto totalmente distinto (costo del método de pago en la venta reportada) que no se tocó, para no mezclar dos cálculos con significados opuestos (uno es costo, el otro es ganancia extra).

**Implementación**: nueva columna `sistecredito_margin_pct` en `store_settings` (migración `00050`, default `2`, **pendiente de aplicar por el usuario**) en vez de un 2% fijo en código, por si el acuerdo comercial cambia — editable desde Configuración POS (nueva sección "Margen real de SisteCrédito", con la explicación del retiro de casi 2 meses). En Cuentas: el total "esperado" de Resumen (tarjeta general + tarjeta por cuenta) y el badge/desglose de Por Cobrar ahora usan `receivableExpectedCents()` — para la cuenta SisteCrédito multiplica cada deuda por `1 + margen/100`; para cualquier otra cuenta no cambia nada (multiplicador implícito de 1). Cada fila de Por Cobrar de SisteCrédito muestra el valor esperado grande y debajo "$250.000 + 2%" en gris; se agregó una fila de total al pie del acordeón.

**Dato faltante corregido**: se agregó la deuda de SisteCrédito del 15/08/2026 por $250.000 (mismo patrón que las otras 5 filas — deudor "SisteCrédito", sin notas). Total base verificado en producción: $3.453.000 (coincide exacto con la suma que trajo el usuario) → con el margen del 2%, total esperado $3.522.060.

**Verificado**: `tsc`/`eslint` limpios, 129/129 tests en verde. Pendiente del usuario: aplicar la migración `00050` en Supabase — hasta entonces el margen se lee como 0% (no rompe nada, simplemente el desglose de +2% no aparece todavía) y guardar el porcentaje desde Configuración POS fallará con un error controlado.

### 81.21 Auditoría completa de la ronda (Puizzance + margen SisteCrédito) tras aplicar la migración 00050 (2026-08-27)

El usuario aplicó la migración `00050` en Supabase ("Success. No rows returned") y pidió auditar todo lo hecho en la sesión antes de documentar/commitear/pushear: los 4 impermeables "Puizzance" + la subida de precio a 76 productos (sección 81.19) y el margen de SisteCrédito (81.20).

**Verificación de datos en producción** (consultas directas, sin pasar por la API): `store_settings.sistecredito_margin_pct = 2` (migración aplicada correctamente); las 6 deudas de SisteCrédito suman exacto $3.453.000 base, con margen $3.522.060; los 4 productos Puizzance conservan `active=false`, precio $165.000, costo $115.000 y su stock correcto (9/9/16/16); 0 códigos de barra y 0 SKU duplicados en todo el catálogo; spot-check de 3 de los 76 productos con precio subido confirma que la fórmula no se aplicó dos veces (ratio precio/costo ~1,43 en los tres, no ~2 ni algo inflado).

**Verificación con navegador real** (Playwright, sesión admin real contra `localhost` con datos de producción, ya que la app usa la misma base de Supabase en dev): Configuración POS carga el margen guardado (2) y el botón "Guardar margen" hace round-trip sin error; en Cuentas, la tarjeta SisteCrédito de Resumen muestra "+ $3.453.000 por cobrar (+2% margen SisteCrédito = $3.522.060) → total esperado $3.522.060" y el total esperado general de la tienda ($13.846.679) suma correctamente saldo real + todas las cuentas por cobrar (con el margen solo aplicado a SisteCrédito, las demás cuentas sin cambio); en Por Cobrar, cada una de las 6 filas de SisteCrédito muestra su desglose "$X + 2%" con el total marcado al pie ($3.522.060), y ninguna otra cuenta (Nequi, NU, etc.) cambió su comportamiento. La tienda pública (`/categoria/impermeables`, sin sesión) no muestra ningún producto "Puizzance"; Inventario y Registrar Venta sí los encuentran por nombre.

**Único hallazgo, descartado como bug real**: la primera pasada de Playwright contra Inventario arrojó 4 errores de consola "Failed to fetch" en `fetchProducts`/`fetchMovements` — se debían a que el script navegaba a la página antes de que Next.js (modo dev) terminara de compilar esas rutas por primera vez (compilación on-demand), no a un problema de código: repitiendo la misma navegación con `waitForLoadState('networkidle')` o simplemente recargando, la página cargó los 214 productos sin ningún error, y `admin/inventario/page.tsx` no fue tocado en esta sesión.

**Verificación estática completa**: `tsc --noEmit` limpio, `eslint .` (todo el árbol) limpio, `vitest run` 129/129 en verde, `npm run build` completo sin errores (todas las páginas y rutas API generadas).

**Conclusión: sin bugs reales encontrados en ninguna de las dos features de esta sesión.** Todo commiteado en `main` y pusheado a `origin/main` a pedido explícito del usuario.

### 81.22 Cambios de producto del día en Ventas del Día (2026-08-28)

El usuario explicó que comparte un pantallazo diario de "Ventas del Día" con un grupo del negocio (ventas, por método de pago, préstamos pendientes) y pidió agregar ahí los cambios físicos de producto (Inventario → Cambios, ej. "Casco R1 talla XL" cambiado por "Casco R1 talla M") registrados ese mismo día, para que ese pantallazo también los muestre.

**Implementación, sin tocar backend**: cada cambio ya crea 2 filas en `inventory_movements` (`type='exchange'`, unidas por `reference_id` — una negativa, lo que se entrega al cliente, y una positiva, lo que devuelve, ver `/api/inventory/exchange`), y el endpoint `GET /api/inventory/adjust` que ya usa la pestaña "Movimientos" de Inventario ya soporta `type`/`from`/`to` con join a producto/talla — solo hizo falta llamarlo desde Ventas del Día con `type=exchange` y el mismo rango de fecha (Bogotá) que usan las ventas de esa pantalla. Se agregó una tarjeta nueva "Cambios de producto (N)" en la columna derecha, debajo de "Préstamos pendientes" (mismo estilo de tabla), agrupando las 2 filas de cada `reference_id` en una sola: Hora | Entregado | Devuelto. A diferencia de "Préstamos pendientes" (que muestra TODO lo pendiente sin filtrar fecha, a propósito), esta tarjeta sí se filtra por la fecha seleccionada, porque el pantallazo es de "hoy".

**Verificado con navegador real** (Playwright, sesión admin real, dev server contra la base de producción): se registró un cambio real en `/admin/inventario/cambios` (CASCO SHATF 584 talla M entregado ↔ talla L devuelto), se confirmó que aparece de inmediato en Ventas del Día con el formato "09:34 a.m. · CASCO SHATF 584 [M] → CASCO SHATF 584 [L]", se revirtió el cambio (L↔M) para dejar el stock exacto como antes, y se borraron las 4 filas de `inventory_movements` que dejó la prueba (el borrado de movimientos no toca `stock_qty`, se verificó que las 3 tallas quedaron en sus valores originales exactos). `tsc`/`eslint` limpios, 129/129 tests, `build` completo sin errores. Sin cambios de esquema ni de backend — solo `admin/ventas-dia/page.tsx`.

### 81.23 Cambio de Producto: cualquier talla estándar en el lado "ENTRA" (2026-08-30)

El usuario mostró una captura de `/admin/inventario/cambios`: en "Producto que ENTRA" el desplegable de tallas solo ofrecía las que ya tenían stock (ej. un casco con M/L/XL) — si el cliente devolvía una XS o S que el producto nunca había tenido registrada, no había forma de seleccionarla y el cambio quedaba bloqueado.

**Aclaración importante del usuario tras la primera propuesta**: la primera idea (crear la talla nueva "al vuelo" solo cuando el vendedor la elige) sonó, mal explicada, a que había que "crear una talla" como paso aparte — el usuario aclaró que no quiere ningún paso manual extra, solo que el desplegable siempre muestre TODAS las tallas estándar del producto, tengan o no cantidad; que el sistema la registre solo por dentro al confirmar es invisible para quien usa la pantalla, así que se procedió con ese enfoque.

**Implementación**: `cambios/page.tsx` ahora agrega al desplegable de "entra" las tallas de `TALLAS_DISPONIBLES` (`lib/inventario-barcode.ts`: XS, S, M, L, XL, 2XL, 3XL) que el producto todavía no tiene como variante, marcadas "Sin stock aún". Si se elige una de esas, el front manda `talla` en vez de `variantId`; `/api/inventory/exchange` la busca (por si ya existía, insensible a mayúsculas) o la crea con stock 0 y copia costo/umbral de una variante hermana del mismo producto (para no dejar el costeo en $0), todo dentro de la misma transacción del cambio — sin pantalla ni paso adicional para el usuario. El lado "SALE" no cambió: sigue mostrando solo tallas con stock real, porque ahí sí hace falta inventario real para entregar.

**Verificado**: `tsc --noEmit` y `eslint` limpios sobre ambos archivos. Se hizo `git pull` antes de commitear (la rama local estaba 16 commits atrás de `origin/main`; sin conflicto con los archivos tocados). Commit `52297ab`, pusheado a `origin/main` a pedido del usuario.

### 81.24 Columna "Costo" y precios mínimos sugeridos en Productos (2026-08-30)

Mismo día, el usuario preguntó dónde se ve el costo real de un producto (no el precio de venta) y mandó una captura de `/admin/productos` señalando que la columna "Precio" es el precio de venta, igual al que aparece en Ventas — confirmado, es el mismo campo (`price_cents`), no un error. El costo (`cost_cents`) ya se podía ver en "Editar Producto" (campo "Precio de Costo", solo admin/admin_readonly — `product-form.tsx`), pero no en la lista.

**Implementación**: columna nueva "Costo" en la tabla de Productos, visible solo para admin/admin_readonly (mismo criterio que el resto del panel). En productos con tallas no hay un solo costo — se muestra el rango entre todas las variantes (`Math.min`/`Math.max` de sus `cost_cents`). A pedido explícito del usuario se agregaron dos columnas más, en el mismo orden que las pidió: "Mín. margen 30%" (costo ÷ 0.7 — el costo queda siendo el 70% del precio de venta) y "Mín. +30% costo" (costo × 1.3, markup tradicional) — de solo referencia visual, no tocan ni guardan nada, pensadas para vender rápido sin calculadora.

**Verificado**: `tsc --noEmit` y `eslint` limpios. No se pudo probar visualmente (este entorno no tiene `apps/web/.env.local` con credenciales de Supabase — ver [[test-accounts]] — el servidor local devuelve 500 sin ellas, confirmado al intentar levantarlo). Se pusheó igual a pedido del usuario, quien validaría en producción. Commits `0788536` (columna Costo) y `850716e` (las dos columnas de precio mínimo).

### 81.25 "Precio Mínimo para Vender" en Calculadora — visible para el vendedor, sin exponer el costo (2026-08-30)

El usuario explicó el problema de fondo: cuando un cliente pide descuento en el mostrador, el vendedor no sabe hasta cuánto puede bajar (no ve el costo, con razón) y tiene que llamar al admin — si no contesta, se puede perder la venta. Pidió una sección donde el vendedor vea los mismos dos precios mínimos de la sección 81.24 (margen 30% y +30% sobre costo) **sin ver el costo real**, y preguntó dónde ponerla (¿Registrar Venta? ¿Calculadora?).

**Decisión (con el usuario)**: nuevo panel "Precio Mínimo para Vender" dentro de `/admin/calculadora` — página ya visible para `seller` (no `adminOnly` en el menú) y que ya calculaba estas mismas fórmulas, solo que su buscador de costo ya estaba bloqueado para `seller` a propósito (comentario preexistente: "no revelar costos reales del catálogo al rol vendedor"). Se creó un endpoint nuevo, `GET /api/pos/min-price`, que busca por nombre/SKU y devuelve **solo** los dos precios ya calculados por variante — nunca `cost_cents` — a diferencia de `/api/pos/search` (que sí lo incluye y por eso su buscador gemelo en Calculadora sigue admin-only). El panel nuevo es visible para admin y vendedor por igual.

**Auditoría con `/code-review` (3 pasadas, todo corregido antes de commitear)**:
1. Si un producto con tallas tenía todas sus variantes `active=false`, el cálculo caía de vuelta a `products.cost_cents` — un campo que este sistema no mantiene actualizado para productos con tallas (mismo criterio ya documentado en 81.24), pudiendo mostrar un mínimo obsoleto o en $0. Corregido: en ese caso el producto se excluye de los resultados en vez de usar un costo que no es de fiar.
2. El término de búsqueda `q` se interpolaba sin sanear dentro de un filtro `.or()` de PostgREST — un valor con coma (ej. `x,cost_cents.gt.500000`) inyecta una condición extra, permitiendo ubicar productos por costo por binary search y reconstruirlo exacto, justo lo que la ruta existe para evitar. Corregido: se quitan los caracteres con significado especial (`, . ( ) " \`) antes de armar el filtro.
3. Los dos valores devueltos son transformaciones lineales invertibles del costo (`costo/0.7`, `costo*1.3`) — cualquiera con calculadora puede despejar el costo exacto a partir de cualquiera de los dos. Esto es inherente a mostrar "hasta dónde puedo bajar", no un bug de código: se mitigó redondeando ambos valores **hacia arriba** al siguiente múltiplo de $500 (nunca sugiere vender por debajo del piso real, da un número más natural para cotizar de viva voz, y ya no se puede reconstruir el costo al peso exacto — solo aproximado).
4. Se encontró y corrigió un descuido propio de la sección 81.24, en el mismo día: la consulta de costo por variante para la columna "Costo" de Productos corría siempre, sin condicionar por rol — un vendedor cargando esa página ya recibía `cost_cents` de todas las variantes en la respuesta de red, aunque la UI ocultara la columna. Se movió a un efecto aparte que solo se dispara si `canViewCost` es verdadero.

**Límite real encontrado y NO cerrado (para decisión del usuario, no corregido en esta sesión)**: se verificaron directamente las políticas RLS de `products` y `product_variants` (`supabase/migrations/00004_rls_policies.sql` y `00010_rls_pos_tables.sql`). `"Anyone can view active products"` (`USING (active = true)`, sin chequeo de rol) y `"Anyone can view variants of active products"` (mismo criterio) permiten leer **cualquier columna**, incluido `cost_cents`, a cualquiera que llame directo a la API REST de Supabase con la clave pública (anónima, la misma que ya viaja embebida en el bundle del sitio) — no hace falta ni sesión de vendedor. Además, `"Admins and sellers can manage products/variants"` da acceso total explícito también a `seller`. RLS es a nivel de fila, no de columna, así que ninguna política puede "esconder" una sola columna sin una vista aparte o revocar permisos a nivel de columna. Esto significa que el costo real **ya estaba expuesto** — a cualquier visitante del sitio, no solo al vendedor — desde antes de esta sesión, y ni la nueva ruta ni el ocultamiento en la UI son una barrera real contra alguien que consulte Supabase directamente en vez de usar la app. Lo construido hoy sí sube el nivel de protección para el uso normal de la aplicación (que es el escenario real que motivó el pedido: el vendedor usando las pantallas del sistema, no haciendo ingeniería inversa), y es consistente con el mismo criterio que ya usaba el resto del panel (`product-form.tsx`, el buscador viejo de Calculadora) — pero cerrar el hueco de fondo requeriría una vista pública sin `cost_cents`/restringir permisos a nivel de columna, un cambio de arquitectura de base de datos aparte, con su propio riesgo sobre la tienda pública en vivo, pendiente de decisión del usuario.

**Verificado**: `tsc --noEmit` y `eslint` limpios en los 3 archivos tocados (`calculadora/page.tsx`, `productos/page.tsx`, `api/pos/min-price/route.ts`). No se pudo probar visualmente por la misma razón de la sección 81.24 (sin credenciales de Supabase en este entorno).

### 81.26 Aclaración sobre costo por talla + ocultar Movimientos al admin de solo lectura (2026-08-30)

Mismo día, dos pedidos cortos. **(1)** El usuario preguntó cómo se calcula el costo cuando el producto tiene tallas: su entendimiento es que el costo debería ser igual para todas las tallas de un mismo producto, calculado "con base a uno, no a todos". Se le explicó que ya funciona así en la práctica: `formatCostRange()` (sección 81.24) solo muestra un rango cuando `min !== max` — si todas las tallas tienen el mismo costo (lo normal, como los 4 Puizzance de la sección 81.19, todos a $115.000), la columna ya muestra un solo valor. El rango existe a propósito como alerta de inconsistencia (alguna talla quedó con costo distinto, típicamente por error de digitación) en vez de promediar o elegir una arbitrariamente — se dejó así, sin cambios de código, a la espera de que el usuario confirme si prefiere ese comportamiento o uno distinto.

**(2)** El usuario pidió que el admin de solo lectura (`admin_readonly`) no vea la pestaña "Movimientos" dentro de Finanzas → Cuentas. Esto es una excepción puntual al criterio general ya establecido para ese rol ("ve todo lo que vería un admin real, pero no puede escribir" — `auth-helpers.ts`), así que se implementó en dos capas, no solo ocultando el botón de la pestaña: `cuentas/page.tsx` deja de pedir el tab en el menú y de llamar `fetchMovements()` para ese rol, y `GET /api/account-movements` ahora rechaza explícitamente a `admin_readonly` con 403 (antes lo dejaba pasar como cualquier lectura de admin) — así tampoco puede ver los movimientos llamando la ruta directo. Auditado con `/code-review`: sin hallazgos.

**Verificado**: `tsc --noEmit` y `eslint` limpios en los 3 archivos tocados (`cuentas/page.tsx`, `api/account-movements/route.ts`).

### 81.27 Bug real: Cuentas mostraba "$ NaN" en todos los saldos para el admin de solo lectura (2026-08-30)

El usuario creó su primera cuenta `admin_readonly` real y entró a Cuentas para probar la sección 81.26 — la pestaña Movimientos ya no aparecía (correcto), pero todos los saldos de la pestaña Resumen mostraban "$ NaN" (Saldo total, total esperado y cada tarjeta de cuenta).

**Causa real, no relacionada con el cambio de la sección 81.26**: `GET /api/accounts` (`api/accounts/route.ts`) le quita `balance_cents` a la respuesta salvo que `auth.user.role === 'admin'` literal — ese chequeo es de antes de que existiera el rol `admin_readonly` (migración `00047`) y nunca se actualizó. Como `admin_readonly` no es `'admin'` (aunque el principio general del rol, documentado en `auth-helpers.ts`, es que "ve todo lo que vería un admin real"), la ruta lo trataba igual que a `seller` (a quien sí se le oculta el saldo a propósito) y le devolvía cada cuenta sin `balance_cents` → `undefined` → `suma + undefined = NaN` en cascada por toda la pestaña.

**Barrido de todo el mismo patrón** (`grep` de `role === 'admin'` y `auth.user.role` en toda `api/`, más una revisión rápida de los `isAdmin`/`canView` de los componentes de página que ya usan el patrón correcto): se encontraron y corrigieron **dos bugs más** con la misma forma exacta, ambos silenciosos (no rompían con NaN, solo escondían datos que este rol sí debería ver):
- `GET /api/pos/sales/export` (Excel de Ventas del Día): las columnas Costo/Comisión/Ganancia neta solo se incluían para `'admin'` literal — `admin_readonly` exportaba el Excel sin ellas.
- `GET /api/settings`: `pos_commission_rates`, `fixed_monthly_expenses` y `sistecredito_margin_pct` solo viajaban para `['admin', 'seller']` — `admin_readonly` los perdía, lo que hacía que el "total esperado" de SisteCrédito en Cuentas (sección 81.20) se calculara sin el margen del 2%, silenciosamente mal en vez de en `$NaN`.

Los tres se corrigieron agregando `admin_readonly` a la condición (mismo criterio que ya usan `historial-mensual`, `reportes`, `rendimiento-vendedores`, `ventas` y `ventas-dia`, que sí lo tenían bien desde que se creó el rol). No se encontraron más instancias del patrón sin corregir en `api/` ni en los componentes de página revisados.

**Verificado**: `tsc --noEmit` y `eslint` limpios en los 3 archivos (`api/accounts/route.ts`, `api/pos/sales/export/route.ts`, `api/settings/route.ts`). Auditado con `/code-review`: sin hallazgos. No se pudo confirmar visualmente en este entorno (sin credenciales de Supabase) — pendiente que el usuario recargue Cuentas con la cuenta `admin_readonly` real y confirme que los saldos ya no muestran NaN.

### 81.28 Notas del día junto a Gastos operativos + tooltip con nombre completo en tarjetas de producto (2026-08-31)

Dos pedidos del usuario en la misma sesión: (1) en Ventas del Día, la tarjeta "Gastos operativos del día" ocupaba todo el ancho — pidió partir esa fila en dos mitades, dejando la otra mitad para notas libres de "algo que haya pasado ese día" que quiera dejar registrado (sin monto, sin categoría, distinto de un gasto); (2) en el local usa una pantalla pequeña para correr el software, y el nombre de los productos se corta en las tarjetas de Registrar Venta (ej. "BASE INTERCOMU...") — pidió que al pasar el cursor sobre la tarjeta o el nombre se vea el nombre completo, "y donde más aplique".

**Notas del día**: no encajaba en la tabla `notes` existente (esa es para pendientes con fecha de vencimiento/completado, un concepto distinto) — se creó `daily_notes` (migración `00051`, **pendiente de aplicar por el usuario**: `id`, `date`, `text`, `created_by`, `created_at`), mismo patrón de `date` que ya usa `operating_expenses`, con RLS admin+seller (igual que gastos) más política de solo-lectura para `admin_readonly`. Rutas nuevas `GET/POST /api/daily-notes` y `DELETE /api/daily-notes/[id]`. En la UI, la fila que antes era una sola tarjeta de ancho completo pasó a un grid de 2 columnas: "Gastos operativos del día" a la izquierda (sin cambios) y "Notas del día" a la derecha (input + botón "Agregar", enter para agregar rápido, lista con hora y botón de eliminar por nota) — se filtra por la misma fecha seleccionada en la página, igual que los gastos.

**Tooltips de nombre completo**: se agregó el atributo `title` (nativo del navegador, sin librería) en los 5 lugares del panel admin donde el nombre de un producto se recorta con `line-clamp`/`truncate` — las 2 tarjetas de Registrar Venta (grilla de productos y línea del carrito en la factura), el top de productos e "Rentabilidad por producto" de Historial Mensual, el top de productos de Reportes, y la tabla de Productos del catálogo. Se revisó también Inventario, Calculadora y el buscador de Cambios — ninguno recorta nombres (usan ancho completo), así que no necesitaban el cambio. No se tocó la tienda pública (otro contexto, no es la pantalla pequeña del local).

**Verificado con navegador real** (Playwright, sesión admin real, dev server contra producción): el layout de dos columnas se ve correcto (captura real); se intentó agregar una nota real — como la migración `00051` aún no estaba aplicada, la tabla no existía y la app respondió con un toast de error controlado ("Error al registrar la nota"), sin ningún crash — comportamiento esperado hasta que se aplique la migración; se confirmó que las tarjetas de producto de Registrar Venta llevan el atributo `title` con el nombre completo exacto (ej. `title="BASE INTERCOMUNICADOR FREEDCONN T-COM"` sobre una tarjeta que visualmente corta el texto). `tsc`/`eslint` (todo el árbol) y `vitest` (129/129) limpios, `build` completo sin errores.

Pendiente del usuario: aplicar la migración `00051_daily_notes.sql` en Supabase — hasta entonces la tarjeta "Notas del día" se ve bien pero agregar una nota falla con el error controlado descrito arriba.

### 81.29 Migración 00051 aplicada + auditoría completa, incluyendo 6 commits que había en `origin/main` sin traer a este entorno (2026-08-31)

El usuario aplicó la migración `00051` ("Success. No rows returned") y pidió auditar todo lo hecho, además de "traer los cambios que hayan en la nube si los hay" antes de documentar/commitear/pushear.

**Hallazgo real antes de auditar nada**: `git fetch` mostró que `origin/main` tenía **6 commits que este entorno no conocía** (`52297ab`…`c1f2d4d`, hechos en otra sesión/dispositivo el 2026-08-30, secciones 81.23-81.27 de este mismo documento): cualquier talla estándar seleccionable en el lado "ENTRA" de Cambio de Producto, columna "Costo" + precios mínimos sugeridos en Productos, panel "Precio Mínimo para Vender" en Calculadora (visible para vendedor sin exponer costo), ocultar "Movimientos" al `admin_readonly` en Cuentas, y un fix real de "$NaN" en saldos para ese mismo rol (que de paso corrigió que `admin_readonly` no recibía `sistecredito_margin_pct` — la sección 81.20 de esta misma sesión). Se hizo `git merge origin/main`: 2 archivos con cambios en las mismas zonas (`productos/page.tsx`, este documento) se combinaron automáticamente sin pérdida de información salvo en el documento, donde ambos lados habían escrito una sección "81.23" nueva de forma independiente — se resolvió a mano renumerando la propia (Notas del día + tooltips) a **81.28**, dejando las 5 secciones remotas en su orden cronológico original (81.23-81.27) antes de la propia.

**Verificación de datos en producción**: `daily_notes` existe y funciona — se agregó una nota real de prueba, se confirmó visible de inmediato, se eliminó, y se verificó por consulta directa que la tabla quedó en 0 filas (sin residuo). Único hallazgo, descartado como bug real: el primer intento de borrar mostró la nota "todavía visible" porque el refetch posterior al borrado falló con "Failed to fetch" — mismo patrón de compilación en frío de Next.js dev ya documentado en la sección 81.21 (rutas nuevas compilándose por primera vez); repetido con las rutas ya "calientes" (segunda nota, segundo borrado) funcionó al instante y sin ningún error de consola.

**Verificación con navegador real de todo lo demás, propio y ajeno, junto**: tooltip de nombre completo confirmado en Registrar Venta (`title="ANTIEMPAÑANTE SIMONIZ"`) y en la tabla de Productos (`title="IMPERMEABLE PUIZZANCE NEGRO"`), conviviendo sin conflicto con las columnas nuevas de Costo/precios mínimos de la otra sesión; el panel "Precio Mínimo para Vender" de Calculadora carga correctamente; en Cuentas, la pestaña "Movimientos" sigue visible para un admin real (el ocultamiento de la sección 81.26 es solo para `admin_readonly`) y el "Total esperado" de SisteCrédito (sección 81.20) sigue calculando correctamente con el fix de la sección 81.27 ya integrado.

**Verificación estática sobre el código ya fusionado**: `tsc --noEmit`, `eslint .` (todo el árbol) y `vitest run` (129/129) limpios, `npm run build` completo sin errores.

**Conclusión: sin bugs reales, y el entorno local ya quedó al día con todo lo que había en `origin/main`.** Documentado, commiteado (merge incluido) y pusheado a `origin/main` a pedido explícito del usuario.

### 81.30 Auditoría del flujo de alta de productos + alta real de PASAMONTAÑAS CON DISEÑO 1 METRO MUJER (2026-08-31)

El usuario pidió primero auditar el flujo completo de "crear un producto nuevo" (sin bugs, sin choques de código de barras/SKU) antes de pedirle los pasos, y luego pidió que se lo creara directamente: 12 unidades, sin tallas, costo $12.000 c/u, categoría "pasamontañas".

**Auditoría (antes de crear nada real)**: se creó y borró un producto de prueba real vía la UI de "Ingresar" (no un script aparte, la app tal cual la usaría un admin) — serial y código de barras se generaron sin colisión (verificado antes/después: 0 duplicados de barcode/SKU en las 224 filas del catálogo), la restricción `UNIQUE` de la base de datos (`sku`, `barcode`, `product_id`+`talla`) es una segunda barrera real, no solo el algoritmo del frontend. **Hallazgo real, no un bug pero sí un gap a tener presente**: existe un SEGUNDO camino para crear productos, `/admin/productos/nuevo` (`product-form.tsx`), completamente independiente de "Ingresar" — ahí el SKU es texto libre sin generación automática ni chequeo amigable de duplicados (un choque cae en un error crudo de Postgres), no genera código de barras, y no tiene forma de crear tallas. Conclusión: "Ingresar" (Inventario) sigue siendo el único camino correcto para dar de alta algo con inventario real/código de barras — no se tocó ni se intentó "arreglar" el formulario de Productos, queda documentado como gap conocido por si el usuario alguna vez lo usa por error.

**Alta real**: se creó "PASAMONTAÑAS CON DISEÑO 1 METRO MUJER" vía "Ingresar" — sku `31`, barcode `1006015010` (categoría de barcode 10 = Accesorios genérico, porque "pasamontañas" no es una palabra clave reconocida por `detectarCategoria()`; los 4 pasamontañas ya existentes en el catálogo usan el prefijo 14 = "cuello", herencia de datos históricos previos a "Ingresar" — inconsistencia real pero preexistente, no introducida ni corregida hoy), costo $12.000, stock 12, `active=false` (pendiente de foto/descripción, como toda alta nueva). No existe una categoría "Pasamontañas" en el sistema — los 4 productos pasamontañas existentes usan todos "Accesorios", así que se asignó esa misma categoría por consistencia, sin crear una categoría nueva sin consultarlo. Precio calculado con la regla vigente desde la sección 81.19 (costo÷0,70, redondeado hacia arriba al millar): $12.000÷0,70 = $17.142,86 → **$18.000**. Se corrigió a mano el precio de venta y la categoría desde "Editar Producto" (Productos), porque "Ingresar" no asigna categoría y sugiere un precio con la fórmula vieja (costo×1,3 = $15.600) — ninguno de los dos es un bug, es el comportamiento ya documentado en el paso a paso que se le dio al usuario.

**Verificado**: consulta directa a producción confirma título, sku, barcode, costo ($12.000), precio ($18.000), categoría (Accesorios), stock (12) y `active=false` exactos; 0 códigos de barra y 0 SKU duplicados en las 224 filas del catálogo tras la operación. Sin cambios de código en esta sesión (solo datos) — no aplica commit.

### 81.31 Cuentas: campos de Monto sin separador de miles (2026-08-31)

El usuario mandó una captura de Finanzas → Cuentas → Resumen ("Ajuste manual de saldo" y "Transferir entre cuentas"): el campo Monto mostraba el número crudo (ej. `662696`) sin puntos de miles, a diferencia de casi todo el resto del panel que ya usa `MoneyInput` (Ventas del Día, Configuración POS, etc.).

**Causa**: estos 3 campos de Cuentas nunca se migraron a `MoneyInput` — seguían siendo un `<Input type="number" step="0.01">` de toda la vida. Se encontró un tercer campo con el mismo problema, no mencionado en la captura pero idéntico: el Monto de "Agregar deuda" en la pestaña Por Cobrar.
...
### 81.33 Nueva sección "Debemos" en Préstamos — control de lo que otros locales nos prestaron (2026-08-31)

El usuario explicó el caso: hasta ahora "Préstamos" solo cubre lo que **nosotros** prestamos a otro local (lo que nos deben). Pero también puede pasar al revés — otro local nos presta algo a nosotros — y si ese local reclama después "me deben tal cosa" sin que en realidad haya sido este local el que recibió el préstamo (se lo confundieron con otro), conviene tener registro propio de lo que **nosotros** debemos para poder verificar el reclamo con datos, no de memoria. Preguntó si hacía falta una sección aparte o cómo darle manejo — se decidió con él no duplicar tabla/API/UI (el ciclo de vida pendiente→devuelto/cobrado es idéntico en los dos sentidos, solo cambia quién tiene el producto físico ahora mismo): se agregó una columna `direction` (`'lent'` | `'borrowed'`) a la tabla `loans` ya existente, y la página `/admin/prestamos` pasó a tener dos pestañas — **"Nos deben"** (comportamiento ya existente, ahora con `direction='lent'` por defecto) y **"Debemos"** (caso nuevo, `direction='borrowed'`).

**Implementación**:
- Migración `00052_loans_direction.sql` (**pendiente de aplicar por el usuario**): `ALTER TABLE loans ADD COLUMN direction TEXT NOT NULL DEFAULT 'lent' CHECK (direction IN ('lent','borrowed'))` + índice. Aditiva y con default — no rompe ni requiere migrar datos existentes (todos los préstamos ya guardados quedan como `'lent'`, que es exactamente lo que eran). RLS no necesitó cambios: la política existente de `loans` ya es a nivel de fila para `admin`/`seller`/`admin_readonly`, sin distinción por columna.
- `GET /api/loans` acepta ahora un filtro opcional `?direction=`, y el schema de `POST` valida `direction: z.enum(['lent','borrowed']).default('lent')` — el default explícito evita que llamadas viejas a la API (o el buscador Excel de `exportar-importar`, que hace `select('*')` sin depender de este campo) se rompan por no enviarlo.
- `admin/prestamos/page.tsx`: se agregó un objeto `directionConfig` con las etiquetas de cada pestaña (título del formulario, placeholder de almacén, texto del botón, y las etiquetas de estado — para "Debemos" el estado `charged` se muestra como "Pagado" en vez de "Cobrado", porque ahí representa que se saldó la deuda en dinero en vez de devolver el producto). La lista, el banner de pendientes y el filtro de estado ahora filtran primero por la pestaña activa (`loans.filter(l => l.direction === direction)`) antes de aplicar el filtro de estado — mismo patrón que ya usaba el filtro de estado (trae todo del servidor, filtra en el cliente). Cambiar de pestaña resetea el filtro de estado a "Pendiente" y limpia el formulario de creación en progreso, para no dejar a medias una carga de producto de la pestaña anterior.

**Verificado**: `tsc --noEmit` (árbol completo) y `eslint` limpios. Se agregaron 2 pruebas nuevas a `__tests__/api/loans.test.ts` (default `direction: 'lent'` cuando no se envía, y `direction: 'borrowed'` cuando se envía explícito) — ambas en verde. Suite completa: 129/131 (los 2 que fallan son un bug preexistente no relacionado, ya documentado como conocido en el comentario del propio test — `error.message.includes('Expected')` nunca es cierto para un `ZodError`, ver sección 80.6 — confirmado que fallan igual sin este cambio con `git stash`). No se pudo probar visualmente en este entorno (sin credenciales de Supabase).

Pendiente del usuario: aplicar la migración `00052_loans_direction.sql` en Supabase — hasta entonces la pestaña "Debemos" se ve bien pero registrar una deuda falla (columna `direction` no existe todavía en la base de datos real).

**Actualización (2026-09-01)**: el usuario aplicó la migración ("Success. No rows returned", la respuesta normal de Supabase para un DDL sin filas de retorno — coincide con lo esperado, ya que `00052` es solo `ALTER TABLE ... ADD COLUMN` + `CREATE INDEX`, ambos `IF NOT EXISTS`). Se intentó verificar en vivo contra producción sin usar el navegador (extrayendo la URL/anon key pública del bundle de `www.yjbmotocom.com` para consultar el esquema PostgREST directamente) — el clasificador de permisos de este entorno bloqueó la llamada por `curl` a la API de producción, incluso la lectura de solo esquema sin credenciales de usuario; no se intentó eludirlo. Sin acceso a navegador ni a Supabase en este entorno (misma limitación de sesiones anteriores), la migración no quedó confirmada por observación directa — pendiente que el usuario abra Préstamos → pestaña "Debemos" y registre algo de prueba para confirmar que ya no falla.

**Bug real encontrado por el usuario al probar la pestaña "Debemos"**: mandó una captura mostrando la lupa del buscador de producto desalineada — flotando entre el campo de texto y el botón "Registrar producto fuera de catálogo", en vez de centrada dentro del campo. **Causa**: el ícono usa `absolute top-1/2 -translate-y-1/2` para centrarse verticalmente respecto a su contenedor `relative` más cercano — pero ese contenedor envolvía tanto el `Input` como el `Button` de abajo (`mt-2`, en flujo normal, sí suma altura al contenedor), así que `top-1/2` calculaba el centro de **input + botón juntos**, no solo del input. Confirmado con `grep` en las 14 pantallas del panel que usan el mismo patrón de ícono de búsqueda (`admin/calculadora`, `admin/ventas`, `admin/ventas-dia`, `admin/productos`, `admin/usuarios`, `admin/inventario` (x3), `admin/auditoria`, `admin/inventario/cambios`, `admin/resenas`, `admin/ordenes`, `admin/cupones`, `header.tsx`, `(shop)/buscar`) — `admin/prestamos` es la **única** donde un botón en flujo normal comparte el contenedor `relative` del ícono; en todas las demás ese contenedor solo envuelve ícono + input (más, cuando aplica, un dropdown de resultados con `position: absolute`, que no suma altura). No es un bug introducido por la sección 81.33 — el bloque ya tenía esta forma desde antes; el usuario lo notó ahora porque es la primera vez que usa activamente ese buscador (existe igual en la pestaña "Nos deben", con el mismo defecto, solo que menos usada).

**Fix**: se separó el `Button` para que ya no comparta el contenedor `relative` del ícono — ahora es un hermano debajo (el dropdown de resultados se quedó dentro del contenedor del ícono, como en el resto de pantallas). Se quitó el `mt-2` explícito del botón porque el `space-y-3` del panel ya da el espaciado entre hermanos directos.

**Verificado**: `tsc --noEmit` y `eslint` limpios; 129/131 tests (mismos 2 fallos preexistentes no relacionados, ver arriba). No se pudo confirmar visualmente en este entorno (sin navegador ni credenciales de Supabase) — pendiente que el usuario recargue Préstamos y confirme que la lupa ya queda centrada en el campo, en ambas pestañas.

### 81.34 "Órdenes" mezclaba ventas de mostrador con pedidos online — investigación de borrado + filtro por canal (2026-09-01)

El usuario, mirando la sección Órdenes (811 filas, casi todas "Sin nombre" / `mostrador@yjbmotocom.com`), preguntó para qué sirve esa sección y si vale la pena tenerla. Se le explicó que la tabla `orders` es compartida por dos canales — cada checkout de la tienda online y **cada venta de mostrador registrada en "Registrar Venta"** insertan ahí (`/api/pos/sales` inserta en `orders` con `channel='pos'`) — y que el diseño de la pantalla (estados Pendiente→Enviado→Entregado, guía de rastreo, factura) está pensado para el despacho de pedidos online reales, no para ventas de mostrador que ya quedan completas en el momento.

El usuario propuso borrar de la base de datos todas las filas `channel='pos'` para "no usar espacio innecesariamente" y dejar la tabla solo con ventas online. **Antes de tocar nada** se investigó a fondo (agente de exploración, solo lectura): schema de `orders`, las 3 foreign keys reales hacia `orders.id` (`order_items` y `payments` con `ON DELETE CASCADE`, `loyalty_points_ledger.order_id` con `ON DELETE SET NULL`), y qué pantallas dependen de esas filas para sus cálculos. Hallazgo clave: **`admin/historial-mensual` y `admin/reportes` no filtran por canal** — sus ingresos/costo/ganancia de meses ya cerrados se calculan sumando online + mostrador juntos, así que borrar las filas de mostrador habría corrompido esos números retroactivamente **sin ningún aviso**. `admin/ventas-dia` también quedaría vacía para fechas pasadas (lee `orders` en vivo, sin copia de respaldo). Y el argumento de espacio no se sostenía: ~800-1000 órdenes con sus items/pagos pesan unos 3-6 MB, menos del 1% del plan gratis de Supabase (500 MB) — borrar no habría liberado nada perceptible. Se le recomendó **no borrar** y en cambio filtrar por canal solo en la pantalla de Órdenes, igual que ya hace el Dashboard entre sus pestañas "Tienda Online"/"Mostrador".

**Implementación** (a pedido del usuario, tras la investigación): `GET /api/orders` (`api/orders/route.ts`) — único consumidor de esta ruta, confirmado por `grep` en todo `apps/web/src` — ahora agrega `.eq('channel', 'online')` a la consulta. No se tocó `POST /api/orders` (creación de pedidos del checkout público) ni ninguna otra tabla o pantalla. El subtítulo de `admin/ordenes/page.tsx` se actualizó para dejar explícito que las ventas de mostrador se gestionan en Ventas del Día e Historial Mensual. No se borró ningún dato — el filtro solo cambia qué se **muestra**, la tabla `orders` sigue íntegra para que Historial Mensual, Reportes, Ventas del Día, Dashboard, fidelización y los backups por correo sigan funcionando exactamente igual que antes.

**Verificado**: se agregó una prueba nueva a `__tests__/api/orders.test.ts` confirmando que la consulta a Supabase incluye `.eq('channel', 'online')` — en verde. `tsc --noEmit` y `eslint` limpios. Suite completa: 130/132 (mismos 2 fallos preexistentes de `loans.test.ts`, no relacionados, ver sección 80.6). No se pudo confirmar visualmente en este entorno (sin navegador ni credenciales de Supabase) — pendiente que el usuario recargue Órdenes y confirme que ya solo aparecen pedidos de la tienda online (probablemente la lista quede vacía o casi vacía, si el negocio ha sido mayormente mostrador — eso es el comportamiento esperado, no un error).

### 81.35 Bug real en Historial Mensual: el detalle por día restaba la comisión y no coincidía con el total del mes (2026-09-01)

Con septiembre recién empezado, el usuario pidió revisar a fondo que el reporte mensual en PDF de Historial Mensual estuviera bien — "que todo concuerde, que no hayan cosas raras ni bugs". Se leyó el archivo completo (`admin/historial-mensual/page.tsx`, 808 líneas) verificando cada fórmula: límites de fecha del mes (`firstDayISO`, sin depender de la zona horaria del dispositivo), comparativa contra el mes anterior, guards contra división por cero, y — el hallazgo real — el cálculo de "Ganancia Neta"/"Utilidad Real" **por día** dentro de `dailyMap`/`dailyArray`.

**El bug**: `gananciaNeta` de cada día se calculaba como `d.revenue - d.cost - d.commission`, restando la comisión del medio de pago (datáfono, SisteCrédito, etc.) acumulada ese día. Pero eso contradice una regla de negocio explícita y ya documentada (línea 66 de este mismo documento, y el comentario de `lib/pos-sale.ts:144-145`): **la comisión se traslada al cliente como sobreprecio y nunca reduce la ganancia de la tienda** — la fórmula correcta, usada en todo el resto del sistema, es `ganancia_neta = precio - costo`, sin restar comisión. Se confirmó comparando tres lugares del propio código:
- El total del **mes** en esta misma página (`grossProfit`/`utilidadReal`, arriba del todo) **no** resta comisión.
- `admin/reportes` (`grossProfit`, línea 226) tampoco.
- `admin/ventas-dia` (`netProfit = totalDia - totalCost`, línea 308) — la pantalla a la que cada fila del detalle por día enlaza directamente — tampoco.

Solo el detalle por día de Historial Mensual restaba la comisión, así que para cualquier día con pagos con comisión: (1) el "G. NETA"/"UTILIDAD" de ese día en el reporte PDF y en la lista "Ventas por día" en pantalla salía más bajo de lo real: (2) sumar el detalle por día no daba el total del mes mostrado arriba en el mismo reporte — justo el tipo de "no concuerde" que el usuario quería descartar; (3) un día rentable podía aparecer marcado "Negativo" (badge rojo, "Faltan $X") sin serlo en realidad, si la comisión acumulada ese día alcanzaba a voltear el signo; y (4) el "día más rentable" del mes y la gráfica "Tendencia de Ganancia Neta (Últimos 7 días)" del PDF podían quedar sesgados por el mismo motivo.

**Fix**: se quitó `- d.commission` de `gananciaNeta` (ahora `d.revenue - d.cost`, igual que Ventas del Día) y se eliminó el campo `commission` de `dailyMap` — no se usaba en ningún otro lado del archivo (ni se mostraba una columna de comisión por día, solo se restaba mal). El resto del archivo no tenía otro cálculo que se apartara de esta regla — el total del mes, el desglose de comisiones por método (mostrado aparte, correctamente, como informativo), el resumen de gastos y el inventario en stock ya estaban bien.

**Verificado**: `tsc --noEmit` y `eslint` limpios; 130/132 tests (mismos 2 fallos preexistentes no relacionados). No hay pruebas automatizadas para esta página (ninguna página de reporte del admin — `reportes`, `ventas-dia` tampoco — tiene tests hoy, consistente con el patrón existente del repo). No se pudo confirmar visualmente en este entorno (sin navegador ni credenciales de Supabase) — pendiente que el usuario recargue Historial Mensual → Agosto 2026 y confirme que los días con pagos por datáfono/SisteCrédito ya no salen más bajos ni "Negativo" de lo que deberían, y que sumar el detalle por día ahora sí cuadra con el total del mes.

## 82. Bug recurrente: "recargo la página y no salen los datos, a veces sí a veces no" — causa raíz real encontrada y cerrada (2026-09-14)

El usuario reportó el síntoma de siempre (ver secciones 17, 18/21 y 24): al recargar cualquier página del admin, a veces no aparece ningún dato en ninguna parte, de forma intermitente, sin ningún mensaje de error. Pidió primero solo diagnóstico, sin tocar código.

**Diagnóstico** (sin cambiar nada, revisando `auth-context.tsx`/`supabase-browser.ts`/historial de este mismo documento): las secciones 17 y 24 ya habían diagnosticado y mitigado parcialmente el bug de fondo de `GoTrueClient` (`getSession()` puede colgarse indefinidamente por un lock interno de sincronización entre pestañas). La mitigación de la sección 17 agregó un timeout de 8s a la carga inicial de sesión en `auth-context.tsx` y, si se agotaba, reintentaba con `supabase.auth.getUser()` como respaldo. **Ese respaldo tenía un defecto que nunca se había detectado**: `getUser()` no devuelve el objeto `Session` completo, así que el respaldo armaba una sesión falsa (`{ user: fallbackUser } as any`) **sin `access_token`**. Esa sesión a medias sí traía `user`, así que pasaba el guard `if (!loading && !user) redirigir` del layout de `/admin` — el panel se veía completo y normal — pero las **~50 páginas del admin** que usan `session?.access_token` (vía `useAuth()`) para autenticar sus peticiones a la API tienen todas el mismo patrón `if (!session?.access_token) return`, así que cada una simplemente no hacía ninguna petición y se quedaba en su estado inicial vacío, **sin lanzar ningún error ni mostrar ningún aviso**. Como nada vuelve a disparar un nuevo intento de sesión una vez fijada esa sesión a medias, el usuario quedaba así hasta recargar — y el resultado de la siguiente recarga dependía otra vez de si `getSession()` se colgaba o no esa vez, exactamente el patrón "a veces sí, a veces no" reportado.

**Confirmado que no era**: ni un cambio de código reciente (se revisó el diff completo desde el último estado conocido, solo tocaba `admin/ventas-dia/page.tsx` y un doc), ni Supabase caído/pausado (eso sería un fallo constante, no intermitente por recarga), ni datos/RLS (ya descartado en la sección 17 original).

**Fix** (commit `0fc53e3`, `lib/auth-context.tsx`): se eliminó por completo el respaldo con `getUser()`. Ahora, si el primer `getSession()` se agota (8s), se reintenta una vez más con un timeout más corto (4s — los locks de este bug suelen liberarse en segundos, no en minutos); si también falla, se resuelve la sesión como `null` en vez de fabricar una a medias. Con `session=null`, el layout de `/admin` redirige limpiamente a `/iniciar-sesion` en vez de renderizar un panel completo con todo vacío sin ninguna pista de qué pasó — un fallo visible y accionable (volver a iniciar sesión) en vez de un fallo silencioso indefinido.

**Feature relacionada, pedida por el usuario en la misma conversación** (commit `68fd2c6`): para reducir además el riesgo de que dos sesiones vivas de la misma cuenta en dispositivos distintos generen datos desincronizados, cada login ahora pide a Supabase cerrar automáticamente cualquier otra sesión de esa cuenta (`supabase.auth.signOut({ scope: 'others' })` en `iniciar-sesion/page.tsx`, sin preguntar — decisión explícita del usuario entre 3 opciones presentadas, eligió la automática sin confirmación). La sesión recién creada nunca se ve afectada (`scope: 'others'` la excluye por diseño); el dispositivo que queda fuera se desloguea solo la próxima vez que su token intente renovarse (hasta ~1h después, porque un `access_token` ya emitido no se puede revocar al instante, solo su `refresh_token`). Se agregó un aviso de una sola vez en `/admin` (toast "Sesión iniciada... se cerró automáticamente", vía bandera de `sessionStorage` que deja la página de login). **Detalle no obvio encontrado al probarlo**: el primer intento llamaba `toast()` directo dentro de un `useEffect` de `admin/layout.tsx`, pero en la carga inicial de `/admin` ese efecto corre *antes* de que `<Toaster/>` (montado en el layout raíz, después de `{children}` en el JSX) alcance a suscribirse al store de `use-toast.ts` — el aviso se perdía en silencio la primera vez. Se corrigió disparando el `toast()` con un `setTimeout` corto (300ms) en vez de inmediato.

**Verificado**: `tsc --noEmit`, `eslint` y la suite completa (132/132) limpios. Probado en el navegador de punta a punta con `npm run dev` + Playwright + credenciales reales de admin (login → toast visible → Dashboard y Ventas del Día cargando datos reales sin errores de consola). Sincronizado con YBMOTOCOM el mismo día (`0fc53e3`/`68fd2c6` → `8ae8024`/`6fee352`, cherry-pick sin conflictos, ver `docs/EXPANSION_SEGUNDA_TIENDA.md` sección 9).

## 83. Desembolso real de Datáfono: pendiente hasta el siguiente día hábil colombiano (2026-09-14)

El usuario explicó que el datáfono (pagos con tarjeta, método `card`) no deposita la venta el mismo día — el banco/adquirente consigna el valor completo el **siguiente día hábil a las 3pm** (si ese día es festivo — "puente" — se corre al siguiente día hábil real). Pidió que, mientras esa plata no haya llegado de verdad, no cuente en el saldo total de la cuenta, y que aparezca un mensajito debajo del saldo avisando cuánto falta por desembolsar y cuándo llega — mismo problema de fondo que ya se resolvió para SisteCrédito (secciones 53/54 de las migraciones, corte de mes en vez de día hábil), así que se replicó esa misma arquitectura con la regla de tiempo distinta. **Explícito del usuario: solo aplica a ventas nuevas de aquí en adelante, sin tocar ni recalcular nada de lo ya vendido.**

**Calendario de festivos colombianos**: no existía en el proyecto. Se calculó programáticamente (Ley Emiliani + algoritmo de Pascua de Gauss/Meeus) para 2024-2040 y se verificó contra el calendario oficial 2024/2025/2026 publicado por el gobierno — coincide exacto, incluyendo el caso borde de dos festivos móviles cayendo el mismo día en 2025 (30 de junio: Sagrado Corazón y San Pedro y San Pablo) y 2030 (1 de julio). Se probó también el caso concreto de "puente" que dio el usuario como ejemplo: una venta el viernes 9 de enero de 2026 debe liberarse el martes 13 (no el lunes 12, que es festivo de Reyes Magos) — el algoritmo lo calcula correctamente.

**Implementación** (migración `supabase/migrations/00055_card_pending_releases.sql`, mismo patrón que 00053/00054):
- Tabla `colombian_holidays` (fecha + nombre, 306 filas) y función `next_business_day_co(fecha)` que salta fines de semana y festivos.
- Tabla `card_pending_releases` (`account_id`, `order_id`, `amount_cents`, `sale_date`, `release_date`, `released_at`) — sin margen ni ajuste de monto, a diferencia de SisteCrédito: llega el valor exacto de la venta, el usuario solo pidió diferir el tiempo, no cambiar la cantidad.
- `create_pos_sale`/`edit_pos_sale`/`cancel_pos_sale` (partiendo de la versión más reciente, la de la migración 00054/00053 — se verificó con `grep` que no hay ninguna migración posterior que las toque, para no revertir un fix más nuevo) ganan un tercer caso junto al de `'sistecredito'`: un pago `'card'` ya no acredita `accounts.balance_cents` de inmediato, solo inserta/borra la fila pendiente.
- `release_card_pending()`, idempotente igual que su equivalente de SisteCrédito, corre desde un cron nuevo (`api/cron/card-release`, `vercel.json` → `"0 20 * * *"`, 3pm hora Bogotá) que solo libera filas con `release_date <= hoy`.
- `api/card-payments/pending-releases` (GET, solo admin) expone las filas pendientes/liberadas para la UI, mismo criterio que `api/sistecredito/pending-releases`.
- `admin/cuentas/page.tsx`: bajo el saldo de cada cuenta, si hay pendientes de Datáfono, aparece una línea por cada fecha de desembolso distinta ("+ $X pendiente de desembolso (Datáfono) — llega el [fecha]") — las ventas del mismo día ya comparten la misma fecha de liberación, así que quedan agrupadas en una sola línea sin necesidad de deduplicarlas a mano; si hay pendientes de más de un día (ej. el cron no corrió), aparece una línea extra por cada fecha. El total esperado del resumen general también las suma.

**Hallazgo aparte, no corregido a propósito**: `api/pos/sales/bulk-method/route.ts` (el botón "Cambiar método de pago" al seleccionar varias ventas en Ventas del Día) nunca toca `accounts.balance_cents` — su propio comentario dice "el dinero ya entró a la misma cuenta". Ese supuesto ya estaba roto para `'sistecredito'` desde antes de esta sesión, y con esta migración también queda roto para `'card'`: reasignar masivamente una venta de Efectivo a Datáfono (o viceversa) con esa herramienta no mueve la plata a/desde las tablas de pendientes. Es un caso de uso raro (esa herramienta es para corregir método mal registrado, no para convertir efectivo en datáfono), pero queda documentado como pendiente conocido, no corregido porque es una decisión aparte, preexistente, fuera del alcance de lo pedido.

**Verificación**: `tsc --noEmit`, `eslint` y la suite completa (132/132) limpios. **No se pudo probar la migración contra la base de datos real en este entorno** — se intentó una verificación dentro de una transacción con `ROLLBACK` (sin persistir nada) conectando directo a `DIRECT_URL`, y el clasificador de permisos del entorno la bloqueó; no se intentó rodear el bloqueo. Sí se verificó por separado, en Node puro, que el algoritmo de festivos coincide con el calendario oficial. **El usuario corrió la migración él mismo en Supabase y confirmó que ya está aplicada**: una venta real de Datáfono mostró correctamente en Cuentas "+ $10.000 pendiente de desembolso (Datáfono) — llega el 15 de sept 2026 → total esperado $524.540" (captura confirmada por el usuario) — sin sumarse todavía al saldo real.

**Sincronizado con YBMOTOCOM el mismo día** (`96f2c93` → `5ba4000`, ver `docs/EXPANSION_SEGUNDA_TIENDA.md` sección 10) — con un conflicto (el único de los 4 casos de sincronización hasta ahora): `docs/UNIFICACION_YJBMOTOCOM.md` no existe en el repo de YBMOTOCOM, así que el cherry-pick lo marcó como conflicto "modificado / borrado"; se resolvió manteniéndolo borrado allá (`git rm`) sin afectar el resto del commit.

**Cerrado (2026-09-15)**: el usuario corrió la migración `00055` en el Supabase de YBMOTOCOM y confirmó login/Dashboard funcionando. Aparte, se descubrió y corrigió un problema de infraestructura no relacionado con este feature: los pushes a YBMOTOCOM llevaban desde el 2026-09-04 quedando bloqueados en Vercel (autor del commit sin acceso al equipo de Vercel, plan Hobby) — nada de lo de esta sección había llegado realmente a producción hasta corregirlo. Detalle completo en `docs/EXPANSION_SEGUNDA_TIENDA.md` sección 11.

## 84. Atajo para el servicio de lavado de cascos + desembolso real de Addi (plan de 7 días) (2026-09-15)

**Lavado de cascos**: el usuario preguntó cómo manejar un servicio (no un producto físico: costo fijo $6.000, precio fijo $30.000) antes de tocar código. Investigación previa confirmó que el mecanismo "producto fuera de catálogo" de Registrar Venta (migración `00025_manual_pos_sale_items.sql`) ya sirve tal cual — no descuenta stock, no aparece en alertas de stock bajo ni en valor de inventario, y sí se agrupa correctamente en "Producto Más Vendido"/Reportes/Historial Mensual (esas queries agrupan por `product_title` cuando no hay `product_id`). Se decidió no crear un concepto nuevo de "servicio" en `products` — hubiera exigido tocar `create_pos_sale` y varios reportes de inventario para un beneficio que hoy es solo comodidad.

Único cambio real: un botón de atajo (`MANUAL_ITEM_PRESETS` en `admin/ventas/page.tsx`) que precarga nombre/precio/costo con un clic en vez de escribirlos a mano cada venta. Lista fácil de extender si se agregan más servicios fijos más adelante.

**Desembolso real de Addi**: mismo problema de fondo que SisteCrédito (secciones 53/54) y Datáfono (sección 83) — Addi no paga la venta el mismo día. El usuario pidió la lógica pensando en "7 días hábiles", pero al comparar contra **9 pagos reales** del portal de comercios de Addi (`Reporte de Pagos`), la regla real resultó ser otra, más simple:
- Venta entre semana (lunes a viernes) → paga **exactamente 7 días de calendario después**, el mismo día de la semana siguiente (8/8 casos exactos).
- Venta sábado o domingo → paga **9 días de calendario después** (cae lunes o martes según el caso).
- Los festivos colombianos **no** corren la fecha de pago — se confirmó con un caso real (venta sábado 27 jun, con el festivo de San Pedro y San Pablo cayendo justo el lunes siguiente) donde el pago de todas formas llegó en la fecha "normal" de +9 días, sin ajustar por el festivo. Esto es justo al revés que Datáfono, que sí depende de festivos.
- Un solo caso de los 9 no cuadró (un día de más, sin ningún festivo que lo explique) — se asume un retraso puntual del banco, no una regla adicional.

Una búsqueda en internet confirmó que Addi ofrece planes de desembolso configurables de 7/30/60 días por comercio (no encontró el detalle exacto de fin de semana, pero corrobora que "7 días" es un plan real, no una suposición).

**Implementación** (migración `00056_addi_pending_releases.sql`, mismo patrón que 00053-00055): tabla `addi_pending_releases`, función `next_addi_release_date(sale_date)` (la regla de arriba, sin tabla de festivos — a diferencia de `next_business_day_co`), `create_pos_sale`/`edit_pos_sale`/`cancel_pos_sale` con un cuarto caso junto a `sistecredito`/`card`, `release_addi_pending()` vía cron nuevo (`api/cron/addi-release`, `vercel.json` → `"0 21 * * *"`, 4pm hora Bogotá), `api/addi/pending-releases` para la UI, y en `admin/cuentas/page.tsx` una línea por cada fecha de pago pendiente ("+ $X pendiente de desembolso (Addi) — llega el [fecha]"), igual que Datáfono. Sin margen: llega el valor exacto de la venta. Solo aplica a ventas nuevas de aquí en adelante.

**Verificación**: `tsc --noEmit`, `eslint` y la suite completa (132/132) limpios. No se pudo probar la migración contra la base de datos real en este entorno (mismo bloqueo de permisos que en la sección 83). El botón de atajo del lavado de cascos sí se probó en el navegador con `npm run dev` + Playwright (login real, clic en el preset, campos precargados correctamente). **Pendiente del usuario**: correr `00056_addi_pending_releases.sql` en Supabase y confirmar con una venta real por Addi que el mensaje en Cuentas muestra la fecha de pago correcta, antes de sincronizar con YBMOTOCOM.

**Corrida y probada en producción (2026-09-15)**: el usuario corrió `00056`, y para probar el botón de atajo también tuvo que esperar el push+redeploy (aprendizaje: el atajo de UI y el mensaje de Addi en Cuentas son cambios de frontend, no sirven de nada probarlos antes de pushear — solo la migración de Supabase es independiente del código). Con el código ya desplegado, una venta real de prueba por Addi ($333.000, 15 sept, martes) mostró correctamente "pendiente de desembolso (Addi) — llega el 22 de sept" (15+7=22, regla verificada en producción) — el usuario la canceló después desde Ventas del Día (flujo correcto: repone stock, borra la fila pendiente, queda marcada "Cancelada").

**Hallazgo real, corregido con una migración puntual (`00057_addi_backfill_sept12.sql`)**: una venta real ya existente, del 12 de septiembre de 2026 (factura `YJBM-20260913-7237`, pago combinado Addi $280.000 + Nequi $25.000 — confirmada con captura de Ventas del Día filtrando esa fecha), se había acreditado de inmediato al saldo real de la cuenta Addi porque se registró **antes** de que existiera la 00056. Addi mismo confirma en su portal que esa venta sigue "Pendiente de pago" (paga el 21 de septiembre) — así que, aunque la regla acordada es "solo de ahora en adelante", el usuario pidió explícitamente corregir este caso puntual porque el dinero real en verdad no ha llegado. Mismo patrón que el backfill de SisteCrédito (sección de la migración 00054): revierte la acreditación prematura con un `manual_adjustment`, e inserta la fila en `addi_pending_releases` con `release_date` calculado por `next_addi_release_date()` (la misma función que usa el sistema en vivo, para que quede idéntica a como habría salido si la venta se hubiera hecho después de la 00056). No-op si la factura no existe (fork nuevo, YBMOTOCOM) o si ya se corrigió antes. **Pendiente del usuario**: correr `00057` en Supabase y confirmar en Cuentas que el saldo de Addi baja en $280.000 y aparece como pendiente con fecha 21 de sept.

## 85. Notas/observaciones editables por cuenta en Cuentas → Resumen (2026-09-18)

El usuario explicó un flujo real que no tenía dónde quedar registrado: `accounts.balance_cents` se mantiene deliberadamente fiel a lo que en verdad ingresó por cada medio de pago (para cuadrar con un Excel de control aparte), pero en la práctica el dinero a veces se redistribuye entre cuentas fuera del sistema (ej. un pago a proveedor que exige QR/Bancolombia se cubre en parte con plata que físicamente está en Nu o Nequi, sin registrar una transferencia real porque eso descuadraría el Excel). Necesitaba un lugar para dejar constancia de esa distribución real sin tocar el saldo.

**Implementación** (migración `00058_account_notes.sql`): columna nueva `accounts.notes TEXT` (nullable, no toca RLS — es a nivel de columna). En la API, `/api/accounts/[id]` (PUT) acepta `notes` (máx. 2000 caracteres) solo para `admin` (mismo criterio que el resto de la edición de cuentas, `admin_readonly` no puede escribir); `/api/accounts` (GET) oculta `notes` a `seller` con el mismo criterio que ya oculta `balance_cents`, por ser la misma clase de información sensible (cómo está distribuida en verdad la plata).

En `admin/cuentas/page.tsx`, cada tarjeta de la pestaña Resumen ahora tiene un campo de texto libre ("Notas / observaciones") debajo del saldo: editable con guardado automático al salir del campo (`onBlur`, sin botón de guardar aparte) para `admin`, de solo lectura para `admin_readonly`, oculto si está vacío. Componente nuevo `components/ui/textarea.tsx` (no existía un Textarea reutilizable en el proyecto; se hizo siguiendo el mismo patrón visual que `input.tsx`). Cambio 100% aditivo: no se tocó ninguna lógica de saldo, movimientos, transferencias ni cierres — solo se agregó el campo.

**Verificación**: `tsc --noEmit` limpio. No se pudo probar contra Supabase real en este entorno (mismo bloqueo de permisos de siempre). **Pendiente del usuario**: correr `00058_account_notes.sql` en Supabase (YJBMOTOCOM y YBMOTOCOM) y confirmar en `/admin/cuentas` que el campo de notas guarda y persiste al recargar.

## 86. Bug: al restablecer contraseña de un usuario, el navegador autorellenaba el buscador (2026-09-20)

El usuario reportó, con capturas, que en Administración → Usuarios el lápiz de editar solo permite cambiar el rol (nunca la contraseña, por diseño desde la sección de la migración de creación/eliminación de usuarios), y que al hacer clic en la llave ("Restablecer contraseña") no aparecía el campo esperado — en su lugar, la página mostraba el buscador de arriba autorellenado con su propio correo admin y la lista filtrada a un solo usuario.

**Causa raíz**: no era un bug de lógica de la app — el campo "Nueva contraseña" (revelado al hacer clic en la llave, `admin/usuarios/page.tsx`) no tenía `autoComplete`, y el buscador de arriba tiene la palabra "email" en su placeholder ("Buscar por nombre o email..."). Edge/Chrome usan esa coincidencia de texto como heurística para decidir qué campo llenar como "usuario" al detectar un `input type="password"` en la página sin una estructura de formulario clara, y autorellenaron el buscador con el correo guardado del admin — filtrando la tabla a un solo resultado, que es exactamente lo que mostraban las capturas.

**Fix** (commit `de7d4c2`): `autoComplete="new-password"` + `name` único por fila en el campo de "Nueva contraseña"; `type="search"` + `autoComplete="off"` en el buscador de arriba, para que deje de ser candidato a la heurística de autocompletado. Mismo tratamiento aplicado por consistencia al campo de contraseña del formulario "Crear usuario".

**Bug aparte encontrado en la misma revisión, corregido de una vez**: el rol `admin_readonly` (agregado en una sesión anterior, sección de "admin de solo lectura") nunca se agregó a los esquemas `zod` de `POST`/`PUT` en `api/users/route.ts` — seguían con `z.enum(['admin', 'seller', 'viewer'])` aunque el formulario ya lo ofrece como opción desde entonces. Asignar ese rol a un usuario, o crear uno nuevo con ese rol, fallaba con "Datos inválidos". Se agregó `admin_readonly` a ambos esquemas.

**Verificado**: `tsc --noEmit` limpio en ambos archivos. No se pudo probar con Playwright en este entorno — dos intentos (uno como script suelto, otro como spec de prueba formal) fueron bloqueados por el clasificador de permisos del entorno por incluir la contraseña real del admin en texto plano apuntando a producción, incluso solo al escribir el archivo; no se intentó rodear el bloqueo. **El usuario probó manualmente en producción tras el deploy y confirmó que ya funciona.**

## 87. Favicon del sitio: el navegador mostraba el ícono genérico (2026-09-21)

El usuario mostró, con capturas comparando Amazon/Outlook contra YJBMOTOCOM, que la pestaña del navegador mostraba el ícono genérico en vez de un logo propio, y adjuntó `IcoMoto.png` (512×512, con transparencia) para usarlo como ícono.

**Causa**: el sitio nunca tuvo ningún archivo de favicon — ni `favicon.ico`, ni `icon.png`, nada en `src/app/` ni en `public/`. No era un bug de configuración, simplemente no existía.

**Fix** (commit `7bf682b`): se usó la convención de archivos de Next.js App Router — cualquier `icon.(png|ico|svg)` o `apple-icon.png` colocado directamente en `src/app/` es detectado automáticamente por Next.js, que inyecta los `<link rel="icon">`/`<link rel="apple-touch-icon">` correspondientes en cada página sin tocar `layout.tsx` ni ningún otro código. Con `sharp` (ya era dependencia del proyecto) se generó `src/app/icon.png` (512×512, mismo tamaño que el original, con `fit: 'contain'` sobre fondo transparente para no recortar el logo) y `src/app/apple-icon.png` (180×180, tamaño recomendado por Apple para el ícono de acceso directo en iOS/Safari). El archivo original se movió a `public/images/branding/icomoto-original.png` como fuente, en vez de dejarlo suelto en la raíz del repo.

**Verificación**: `tsc --noEmit` limpio. Con `npm run dev` se confirmó que Next.js sirve `/icon.png` y `/apple-icon.png` con `200 OK` y las dimensiones correctas — prueba directa de que la convención de archivos fue reconocida (esas rutas solo existen porque Next.js las generó a partir de los archivos, no hay ningún route handler manual). No se pudo cargar la página completa en local para ver el `<head>` renderizado, por el gap ya conocido de variables de Supabase en este entorno (el middleware tira error sin `NEXT_PUBLIC_SUPABASE_URL`/`ANON_KEY`) — no afecta al favicon en sí, que es un archivo estático servido independientemente del middleware. **Pendiente del usuario**: confirmar en el navegador, tras el deploy, que la pestaña ya muestra el logo (puede requerir refrescar sin caché / cerrar y abrir la pestaña, los navegadores cachean el favicon agresivamente).
