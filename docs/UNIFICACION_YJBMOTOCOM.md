...
**⚠️ Pendiente manual antes de que "Nuevo Arqueo" funcione en producción**: aplicar `supabase/migrations/00041_cierres_arqueo_fisico.sql` en el SQL Editor de Supabase. Hasta entonces, guardar un cierre nuevo fallará (las columnas `cash_expected_cents`/`cash_difference_cents` no existen todavía en la base real) — el historial y el resto del panel siguen funcionando con normalidad mientras tanto.
