...
**Verificación**: `tsc --noEmit`, `eslint` y la suite completa (132/132) limpios. **No se pudo probar la migración contra la base de datos real en este entorno** — se intentó una verificación dentro de una transacción con `ROLLBACK` (sin persistir nada) conectando directo a `DIRECT_URL`, y el clasificador de permisos del entorno la bloqueó; no se intentó rodear el bloqueo. Sí se verificó por separado, en Node puro, que el algoritmo de festivos coincide con el calendario oficial. **El usuario corrió la migración él mismo en Supabase y confirmó que ya está aplicada** (mismo flujo que 00053/00054) — pendiente que confirme con una venta real de Datáfono que el mensajito en Cuentas muestra la fecha correcta antes de sincronizar con YBMOTOCOM.
