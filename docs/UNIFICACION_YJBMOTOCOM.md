...
**No corregido en el momento** — es un cambio de comportamiento financiero real (a qué cuenta le llega el dinero) y requiere una decisión de negocio del usuario, no solo un fix técnico: confirmar que el fix correcto es igualar la regla del local (tarjeta → cuenta NU) antes de tocar código que mueve saldos de cuentas reales. Candidato inmediato para la próxima sesión.
