...
**Fase 4.1 (seguridad y permisos) completa — ver 13.1.1.** Seguir por **13.2 (Fase 4.2, cálculos financieros)** en orden 4.2.1 → 4.2.6. Cada ítem se implementa, se verifica, y se documenta aquí mismo (reemplazando "Acción propuesta" por lo realmente hecho, igual que se hizo en las Fases 2/3/3B/4.1) antes de pasar al siguiente. No se hace `git push` en ningún punto sin autorización explícita del usuario. El único bloqueo conocido de antemano es **4.3.1** (necesita PDFs de muestra reales del usuario) — se puede saltar y volver a él al final de la Fase 4.3 sin bloquear el resto.

**Pendientes manuales acumulados de la Fase 4**: aplicar `00019_cuentas_admin_only.sql` (Fase 4.1) en el SQL Editor de Supabase.
