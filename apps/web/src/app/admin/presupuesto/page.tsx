...
import { PiggyBank, Plus, Loader2, Trash2, Receipt } from 'lucide-react'
          ...
          {budgets.length === 0 ? (
            ...
            <div className="space-y-2">
              {budgets.map((b) => {
                ...
                const pct = b.budgeted_amount_cents > 0 ? Math.min((spent / b.budgeted_amount_cents) * 100, 100) : 0
                ...
                return (
                  <div key={b.id} className="rounded-xl border bg-card p-4">
                    <div className="flex items-center justify-between">
                      ...
                      <p className={cn('text-sm font-medium', over && 'text-red-500')}>
                        {formatPrice(spent)} / {formatPrice(b.budgeted_amount_cents)}
                      </p>
                    </div>
                    <div className="mt-2 h-2 overflow-hidden rounded-full bg-muted">
                      <div
                        className={cn('h-full rounded-full', over ? 'bg-red-500' : 'bg-cyan-500')}
                        style={{ width: `${pct}%` }}
                      />
                    </div>
                  </div>
